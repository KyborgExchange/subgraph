 ...
 * @param event
 */
export function updateKyborgDayData(event: ethereum.Event): KyborgDayData {
  const kyborg = Hub.load(HUB_ADDRESS)!
  const timestamp = event.block.timestamp.toI32()
  const dayID = timestamp / 86400 // rounded
  const dayStartTimestamp = dayID * 86400
  let kyborgDayData = KyborgDayData.load(dayID.toString())
  if (kyborgDayData === null) {
    kyborgDayData = new KyborgDayData(dayID.toString())
    kyborgDayData.date = dayStartTimestamp
    kyborgDayData.volumeETH = ZERO_BD
    kyborgDayData.volumeUSD = ZERO_BD
    kyborgDayData.volumeUSDUntracked = ZERO_BD
    kyborgDayData.feesUSD = ZERO_BD
  }
  kyborgDayData.tvlUSD = kyborg.totalValueLockedUSD
  kyborgDayData.txCount = kyborg.txCount
  kyborgDayData.save()
  return kyborgDayData
}
