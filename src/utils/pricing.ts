...
import { exponentToBigDecimal, safeDiv } from '../utils/misc'
import { Bundle, Pool, Tier, Token } from './../types/schema'
import { ONE_BD, ONE_BI, STABLE_COINS, USDC_ADDRESS, WETH_ADDRESS, WHITELIST_TOKENS, ZERO_BD, ZERO_BI } from './constants'
...
export function sqrtPriceX72ToTokenPrices(sqrtPriceX72: BigInt, token0: Token, token1: Token): BigDecimal[] {
  const num = sqrtPriceX72.times(sqrtPriceX72).toBigDecimal()
  const price1 = num
    .div(PRICE_DENOM)
 ...
 */
const USDC_WETH_POOL_TIER_ID = 1
...
export function getEthPriceInUSD(): BigDecimal {
  ...
  const usdcTier = Tier.load(USDC_WETH_POOL_TIER)
  if (usdcTier !== null) {
    ...
    for (let i = 0; i < whiteList.length; ++i) {
      ...
      for (let j = 0; j < pool.tierIds.length; ++j) {
        ...
        if (tier.token1 == token.id) {
          const token0 = Token.load(tier.token0)!
          // get the derived ETH in pool tier
          const ethLocked = tier.amount0.times(token0.derivedETH)
